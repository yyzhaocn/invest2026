...
def main():
    ...
    ap.add_argument("--type", default="概念", choices=TYPES,
                    help="板块类型：概念 / 行业（一级+二级+三级合并）/ 一级行业 / 二级行业 / 三级行业")
    ...
    if query:
        print(f"搜索 {query!r}（{args.type}，匹配 {len(rows)} 条）:")
    else:
        print(f"{args.type}板块（共 {len(rows)} 个，按涨跌幅降序）:")

    show_level = args.type == "行业"
    header = pad("代码", 10) + pad("名称", 18) + (pad("级别", 6) if show_level else "") \
        + pad("今日涨跌幅", 12, "right") + " 领涨股"
    print(header)
    print("-" * display_width(header))
    for r in shown:
        pct_str = f"{r['pct']:+.2f}%" if r.get("pct") is not None else "--"
        level = pad(r.get("level", ""), 6) if show_level else ""
        print(pad(r["code"], 10) + pad(r["name"], 18) + level
              + pad(pct_str, 12, "right") + " " + str(r.get("leader", "")))
    if args.top > 0 and len(rows) > args.top:
        print(f"… 共 {len(rows)} 条（用 --top 0 显示全部）")
