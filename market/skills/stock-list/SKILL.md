...
- `查询词`（必填）：股票代码前缀（如 `688256`、`6005`）或名称子串（如 `寒武纪`、`平安`）。
- `--top N`：最多显示 N 条（默认 15）。
- `--market`：市场过滤（模糊包含），如 `沪`、`深`、`科创`、`创业`、`北`。
- `--json`：输出 JSON。
- 默认排除三板与退市股，如需包含用 `--all`。
...
- **代码列表与涨跌幅**：本地最新行情快照 `generated/em/<date>/quote_<date>_latest.csv`（全市场约 5300 只，含当日涨跌幅）；快照缺失时仅返回 suggest 结果
- 快照日期较旧时涨跌幅仅供参考
